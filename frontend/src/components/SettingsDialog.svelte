...
<Dialog.Root bind:open>
  <Dialog.Portal>
    ...
    <Dialog.Content
      class="anim-dialog fixed top-1/2 left-1/2 z-50 flex h-[min(600px,88dvh)] w-[min(720px,94vw)] -translate-x-1/2 -translate-y-1/2 flex-col rounded-lg border border-edge bg-surface shadow-2xl"
    >
      ...
      {#if settings}
        <Tabs.Root bind:value={tab} orientation="vertical" class="flex min-h-0 flex-1">
          <Tabs.List class="flex w-44 shrink-0 flex-col gap-1 p-2">
            {#each [{ value: "connection", label: m.connection(), Icon: Cable }, { value: "behavior", label: m.behavior(), Icon: SlidersHorizontal }, { value: "language", label: m.language(), Icon: Languages }, { value: "obs", label: m.obs(), Icon: Video }] as tabItem (tabItem.value)}
              <Tabs.Trigger
                value={tabItem.value}
          ...
          <div class="min-h-0 flex-1 overflow-y-auto p-4">
            ...
            <Tabs.Content value="language"><LanguageSection {settings} /></Tabs.Content>
